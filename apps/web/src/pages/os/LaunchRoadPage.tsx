...
const zoneBlueprint: Array<{ id: number; label: string; subtitle: string; mission: string; stepId?: number; leftPct: number; topPct: number }> = [
  { id: 1, label: 'Idea Valley', subtitle: 'Raw Idea', mission: 'Turn your raw concept into a clear business direction.', stepId: 1, leftPct: 8, topPct: 82 },
  { id: 2, label: 'Brand Forge', subtitle: 'Identity & Voice', mission: 'Craft naming, tone, positioning, and visuals.', stepId: 2, leftPct: 20, topPct: 70 },
  { id: 3, label: 'Business Case Bridge', subtitle: 'Strategy', mission: 'Define pricing, economics, and competitive plan.', stepId: 3, leftPct: 34, topPct: 59 },
  { id: 4, label: 'Preview Studio', subtitle: 'Live Preview', mission: 'Generate the first customer-facing experience.', stepId: 4, leftPct: 47, topPct: 51 },
  { id: 5, label: 'U.S. Setup Gate', subtitle: 'Compliance', mission: 'Complete setup checklist and readiness verification.', stepId: 5, leftPct: 61, topPct: 45 },
  { id: 6, label: 'Funding Mountain', subtitle: 'Capital Prep', mission: 'Build funding story, assumptions, and doc pack.', stepId: 6, leftPct: 73, topPct: 36 },
  { id: 7, label: 'Launch City', subtitle: 'Go Live', mission: 'Execute launch board, CRM, and service operations.', stepId: 7, leftPct: 85, topPct: 28 },
  { id: 8, label: 'Growth Command Center', subtitle: 'Scale', mission: 'Track KPI rhythm and optimize for recurring growth.', leftPct: 92, topPct: 20 },
]
...
export default function LaunchRoadPage() {
  ...
  return (
    <div className="space-y-4 overflow-x-hidden">
      <section className="lr-panel-premium lr-crimson-glow p-5">
        <p className="text-xs uppercase tracking-[0.18em] text-[var(--bs-gold)]">THE SHEEP Launch Road</p>
        <h1 className="text-2xl md:text-3xl font-bold mt-1">{projectName}</h1>
        <p className="text-sm text-[var(--fsi-text-muted)] mt-2">Navigate the premium launch world from Raw Idea to Growth Command Center with AI guidance, checkpoints, and unlocks.</p>
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          <Link to="/modules" className="rounded-full border border-[var(--fsi-border)] px-4 py-2 text-sm hover:border-[var(--bs-gold)] transition">Full AI Module Library</Link>
          <Link to="/assets" className="rounded-full border border-[var(--fsi-border)] px-4 py-2 text-sm hover:border-[var(--bs-gold)] transition">Open Asset Vault</Link>
          <Link to="/marketplace" className="rounded-full border border-[var(--fsi-border)] px-4 py-2 text-sm hover:border-[var(--bs-gold)] transition">Marketplace Power-Ups</Link>
        </div>
      </section>

      <div className="grid xl:grid-cols-[1.55fr,0.85fr] gap-4 items-start">
        ...
        <div className="space-y-3 min-w-0">
          ...
          <div className="lr-panel-premium p-3 grid grid-cols-2 gap-2">
            <div className="rounded-xl border border-[var(--fsi-border)] bg-black/20 p-3">
              <p className="text-xs uppercase tracking-[0.14em] text-[var(--fsi-text-dim)]">Launch Score</p>
              <p className="text-2xl font-semibold mt-1">{progressPct}%</p>
            </div>
            <div className="rounded-xl border border-[var(--fsi-border)] bg-black/20 p-3">
              <p className="text-xs uppercase tracking-[0.14em] text-[var(--fsi-text-dim)]">Readiness</p>
              <p className="text-2xl font-semibold mt-1">{readinessPct}%</p>
            </div>
            <div className="rounded-xl border border-[var(--fsi-border)] bg-black/20 p-3">
              <p className="text-xs uppercase tracking-[0.14em] text-[var(--fsi-text-dim)]">AI CEO Brief</p>
              <p className="text-sm mt-1">{runs.length} runs • {activity.length} activities</p>
            </div>
            <div className="rounded-xl border border-[var(--fsi-border)] bg-black/20 p-3">
              <p className="text-xs uppercase tracking-[0.14em] text-[var(--fsi-text-dim)]">Mission Support</p>
              <p className="text-sm mt-1">{openSupport} open requests</p>
            </div>
          </div>
