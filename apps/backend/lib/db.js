...
const PLACEHOLDER_PATTERNS = ['PASTE_YOUR_', 'your-supabase', 'your-service-role']
...
function buildSupabaseClient() {
  ...
  // Reject obvious placeholder values so we don't fake a "connected" state
  if (key.length < 50 || PLACEHOLDER_PATTERNS.some(p => key.includes(p))) {
    console.warn('[db] ⚠️  SUPABASE_SERVICE_KEY looks like a placeholder — using memdb.')
    console.warn('[db]    Get the real key: Supabase Dashboard → Project Settings → API → service_role')
    return null
  }
