...
const { generateAI, parseAIJson, getActiveProvider } = require('../lib/aiRouter')
...
Please diagnose this business problem and provide a structured fix strategy.`
    ...
    if (isUsingSupabase()) {
      ...
      db.from('fixer_diagnoses').insert(record).then(() => {}).catch(err =>
        console.warn('[fixer] DB save failed (non-blocking):', err.message)
      )
    }

    return res.json({ result: parsed, demo: false, provider: getActiveProvider() })
  } catch (err) {
